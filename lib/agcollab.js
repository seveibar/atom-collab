...
module.exports = function(){
    ...
    agc.joinedSession = function(e){
        ...
        if (e.sessionSize < 2){
            ...
            if (session.isArchivedSession(e.sessionID)){
                ...
                session.load(e.sessionID, function(success){
                    if (session.getStateRevision() == e.stateRevision && success){
            ...
            session.load(e.sessionID, function(){
    ...
    agc.doFileDone = function(e){
        var filePath = ui.getOpenFilePath({relative:true});
        session.fileDone(filePath, function(changeInfo){
            server.sendModifiedSession(changeInfo);
        });
    };

    agc.sessionModified = function(changeInfo){
        session.loadModification(changeInfo);
    };
    ...
    function init(){
        ...
        ui.addCommand("file-done", agc.doFileDone);
